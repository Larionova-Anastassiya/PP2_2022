...
class Coin(pygame.sprite.Sprite):
    def __init__(self):
        super().__init__()
        self.image = pygame.transform.scale(pygame.image.load("coin.png"), (50, 50)) #картинка противника
...
# Создание групп при столкновении с монеткой sprite
...
while running:
    ...
    # Перемещает и перерисовывает все спрайты
    for entity in all_sprites1:
        DISPLAYSURF.blit(entity.image, entity.rect)
        entity.move()

    # если произойдет столкновение между Игроком и coin
    if pygame.sprite.spritecollideany(P1, enemies1):
        DISPLAYSURF.blit(background, (0, 0))
        new_score += 1
